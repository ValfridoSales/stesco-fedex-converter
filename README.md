...
A browser-only converter for turning Flute `.xls`, `.xlsx`, or `.csv` exports into the 70-column FedEx batch CSV format used by the customer service team.
...
1. Drop one or more Flute `.xls`, `.xlsx`, or `.csv` files onto the upload area. Formats can be mixed in one batch; use Add files to extend it.
...
Each review row shows its source filename, source format, and row number. The first worksheet is used for Excel workbooks. If any newly selected file is invalid, none of that selection is added; the existing batch remains intact. Repeated PO numbers alone do not count as duplicates. Clear starts a new batch.
...
Each selected Flute file is shown separately in the results area with an XLS, XLSX, or CSV badge, filename, and shipment-line count. When all rows pass validation and duplicate review, use **Download FedEx CSV** to create the single combined upload file.
